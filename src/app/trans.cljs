...
(def romhir {"a" "\u3042" ,"i" "\u3044" ,"u" "\u3046" ,"e" "\u3048" ,"o" "\u304A",
             ...
             "wa" "\u308F","wo" "\u3092","n" "\u3093"
             }); almost full unicode hiragana table; x stands for small kana (see cykana and sokuon below)
...
(def hgp
	(insta/parser
     "<mot>=h+
        ...
        <wspace> = (' ')*|'.'|','|'?'|'!'
        cykana  = conson ykana
        sokuon = conson &conson" )) ;parse errors tested with insta/failure?
...
(defn hir [x] (apply str (map tokana (tr2 (tr x)))));  transform latin character chain to hiragana
;; ================== Interface =========================
(defn transl[chaine]
   (if (insta/failure? (hgp chaine))
      "Waiting"
      (hir chaine)))
